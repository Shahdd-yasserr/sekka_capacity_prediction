...
def validate_payload(payload: dict) -> dict:
    ...
    for field, rules in SENSOR_SCHEMA.items():
        ...
        if not isinstance(raw, rules["type"]):
            try:
                cleaned[field] = rules["type"] if not isinstance(rules["type"], tuple) else rules["type"][0]
                cleaned[field] = (rules["type"][0] if isinstance(rules["type"], tuple) else rules["type"])(raw)
    ...
    if isinstance(ts, str):
        ...
        raise ValidationError(f"Timestamp must be a string or datetime, got {type(ts)}")
...
class StationLagCache:
    ...
    def get_lag_features(self, station_id: int) -> dict[str, float]:
        ...
        return {
            ...
            "rolling_1h_mean":    _rolling_mean(8),
            "rolling_2h_mean":    _rolling_mean(16),
        }
...
def get_lag_features(self, station_id: int) -> dict[str, float]:
        ...
        def _lag(n: int) -> float:
            # lag-n means n readings ago (index from the right)
            return counts[-n] if len(counts) >= n else fallback
...
class SensorBuffer:
    ...
    def ingest_batch(self, payloads: list[dict]) -> dict:
        ...
        Returns summary: {"accepted": N, "rejected": N, "errors": [...]}
        """
    ...
    def check_dropouts(self, expected_station_ids: list[int], current_time: pd.Timestamp) -> list[int]:
        """
        Compare which stations haven't reported recently.
        Marks them in the lag cache and returns list of offline station IDs.
        """
        offline = []
        cutoff = current_time - pd.Timedelta(minutes=15)  # 2 missed intervals
    ...
    def interpolate_dropout(self, station_id: int, timestamp: pd.Timestamp) -> float:
        """
        Estimate people_count for an offline station using:
          1. Same station, same hour, recent days (preferred)
          2. Global fallback mean
        """
        if self.window_df.empty:
            return 30.0
    ...
    def get_window(self) -> pd.DataFrame:
        """Return the full rolling window DataFrame (copy)."""
    ...
    def mark_retrained(self) -> None:
        """Call this after every successful retrain."""
    ...
    def _prune(self) -> None:
        ...
        self.window_df = self.window_df[
            self.window_df["timestamp"] >= cutoff
